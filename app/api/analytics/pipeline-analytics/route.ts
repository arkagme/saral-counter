...
function categorizeError(raw: string): string {
  ...
  if (r.includes("str' object has no attribute")) return "Script attribute error";
...
function processPipelineDoc(docId: string, data: Record<string, unknown>) {
  const paper: {
    ...
    last_successful_stage: (data.last_successful_stage as string) || "",
    artifacts: {},
    stages: {},
  };

  // Extract per-stage info from flat keys (stages.xxx)
  ...
  for (const [artifact, stages] of Object.entries(ARTIFACT_STAGES)) {
    ...
    const cs = (data.current_stage as string) || "";
    const failedStageName = cs.endsWith("_failed")
      ? cs.replace("_failed", "")
      : "";
    const isThisArtifactFailed =
    ...
    // Fallback: check current_stage _failed pattern
...
export async function GET(request: NextRequest) {
  ...
  try {
      ...
      if (cacheDoc.exists) {
        ...
        if (Date.now() - cachedAt < CACHE_TTL_MS) {
          ...
          chunksSnap.docs.forEach((doc) => {
            const d = doc.data();
            if (d.papers) papers.push(...d.papers);
          });
    ...
    // Full fetch from Firestore (all paper_pipeline docs)
    ...
    // Delete old pipeline chunks (batched)
    const oldChunks = await cacheRef.collection("pipeline_chunks").get();
    if (oldChunks.size > 0) {
      for (let i = 0; i < oldChunks.docs.length; i += 500) {
        const batchDel = db.batch();
        oldChunks.docs.slice(i, i + 500).forEach((d) => batchDel.delete(d.ref));
